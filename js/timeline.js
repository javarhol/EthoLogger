...
(function () {
    ...
    function computeLanes(proj) {
        ...
        // Collect all behavior IDs that appear in annotations
        var annotatedIds = {};
        var annotations = (proj && proj.annotations) || [];
        for (var a = 0; a < annotations.length; a++) {
            if (annotations[a].behaviorId) {
                annotatedIds[annotations[a].behaviorId] = true;
            }
        }
    ...
    function _drawLaneLabels(ctx, height) {
        ...
        for (var i = 0; i < lanes.length; i++) {
            ...
            // Draw category header if this is the first behavior in the category
    ...
    function _drawAnnotations(ctx, annotations, currentTime, width) {
        ...
        // Build a quick lookup from behaviorId to lane index
        var laneLookup = {};
        for (var i = 0; i < lanes.length; i++) {
            laneLookup[lanes[i].behaviorId] = i;
        }
        ...
        for (var a = 0; a < annotations.length; a++) {
            ...
            var laneIndex = laneLookup[annotation.behaviorId];
